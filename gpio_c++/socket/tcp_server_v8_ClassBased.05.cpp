...
class Connection {
public:
    Connection(int socket, sockaddr_in addr, int threadId) 
        : socket_(socket), address_(addr), m_threadId(threadId) {}
...
public:
    void addConnection(int socket, sockaddr_in addr) {
        ...
        auto connection = std::make_shared<Connection>(socket, addr, ++threadId);
        ...
        // connectionMap[threadId] = std::thread([this, connection]() {
        //     connection->handle();
        //     removeConnection(connection);
        // });

        connectionMap[threadId] = connection; //add a pair of <key,value> to the map
    ...
    void printActiveConnections() const {
        ...
        for (const auto& [threadId, conn] : connectionMap)
            std::cout << conn->getClientInfo() << ", threadId=" << threadId << std::endl;       
    }
    ...
    void removeConnection(int threadId) 
    {
        ...
        // }
        auto it = connectionMap.find(threadId);
        if (it != connectionMap.end()) {
            std::cout << "Found connection, threadId=" << threadId << ", removing it...\n";
...
private:
    ...
    std::map<int, std::shared_ptr<Connection>> connectionMap;
