...
public:
    ...
    void handle(std::function<void(std::shared_ptr<Connection>)> const& lambda) 
    {
...
public:
    void addConnection(int socket, sockaddr_in addr) {
        ...
        auto callback = [this](std::shared_ptr<Connection> conn)
        {
            std::cout << "callback to remove connect\n";
            removeConnection(conn);
        };

        connectionMap[connection] = std::thread(&Connection::handle, connection, callback);
    }
    ...
    void removeConnection(shared_ptr<Connection> connection) 
    {
        ...
        if (it != connectionMap.end()) {
            std::cout << "Found connection, removing it...\n";
